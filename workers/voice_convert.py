...
def fallback_pitch_shift(input_path, output_path, semitones):
    """Fallback: pitch shift + basic voice effects
    When no voice conversion tool is available, apply pitch shift
    (minimum 4 semitones so the result is audibly different)
    and add a subtle echo for a richer effect."""
    import librosa
    import soundfile as sf
    import numpy as np
    ...
    # Ensure pitch shift is noticeable (min 4 semitones)
    if abs(semitones) < 4:
        semitones = 7 if semitones >= 0 else -7
        print(f'  [convert] Boosting pitch shift to {semitones} st (no voice tool available)')

    shifted = librosa.effects.pitch_shift(y=audio, sr=sr, n_steps=semitones)

    # Add subtle echo for richer demo sound
    delay_samples = int(0.15 * sr)  # 150ms delay
    decay = 0.3
    echoed = np.copy(shifted)
    echoed[delay_samples:] += decay * shifted[:-delay_samples]
    # Normalize
    max_val = np.abs(echoed).max()
    if max_val > 0.99:
        echoed = echoed / max_val * 0.95

    output_dir = os.path.dirname(output_path)
    os.makedirs(output_dir, exist_ok=True)
    sf.write(output_path, echoed, sr)

    print(f'  [convert] Pitch shift + echo complete: {semitones} semitones')
    return output_path
